...
public class Archer {
    ...
    private static final long SHOOT_INTERVAL = 900;

    private int arrowsLeft = MAX_ARROWS;
    private long lastShot = 0;
    public boolean dead = false;

    private final SpriteAnimation shootAnim;
    ...
    public void update(List<Enemy> enemies){
        ...
        if (now - lastShot > SHOOT_INTERVAL){
            ...
            Enemy target = enemies.stream().filter(e -> !e.isDead()).min((a,b) -> Double.compare(a.x, b.x)).orElse(null);
            if (target != null){
    ...
    public void draw(GraphicsContext gc){
        ...
        gc.fillRedt(x, y-10, W, 6);
        gc.setFill(Color.GOLD);
        gc.fillRedt(x, y-10, W * arrowsLeft/MAX_ARROWS, 6);
